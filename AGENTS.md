...
# Cloud Functions
npm --prefix functions run build   # TypeScript check de Cloud Functions
firebase deploy --only functions   # Deploy de funciones callable/API
...
- **Server logic:** Firebase Cloud Functions (Node.js 20 + Admin SDK)
...
│   └── paymentInstances.ts  # Utilidades legacy/actualización de instancias
├── pages/             # Páginas principales (Dashboard, Cards, Payments, etc.)
└── utils/             # Animaciones y contexto de períodos
/functions
└── src/index.ts       # Cloud Functions callable/API
```
...
- **Generación automática:** La callable `ensurePaymentInstances` en `functions/src/index.ts` garantiza instancias del mes actual y siguiente de forma idempotente en servidor. El cliente no debe crear automáticamente `payment_instances` desde el Dashboard.
...
- `/functions/src/index.ts` - Cloud Functions, incluyendo `ensurePaymentInstances`
